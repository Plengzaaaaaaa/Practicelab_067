...
wrRoute.post('/users', function (req, res, next) {
    ...
    connection.execute(`INSERT INTO users_tbl
     (id,name, tel,zone, username,surname, password, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?,?,?,?);`, [req.body.id, req.body.name, req.body.tel, req.body.zone, req.body.username, req.body.surname, mypass, currentDateTime, currentDateTime])
        .then(() => {
            console.log('ok');
